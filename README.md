...
# 工程内容
算法基于tensorflow 1.12实现论文`Rich feature hierarachies for accurate object detection and semantic segmentation`
# 开发环境
...
因此paper采用的方法是：首先输入一张图片，我们先定位出若干个物体候选框，然后采用CNN提取每个候选框中图片的特征向量，特征向量为全连接层去处顶层后的输出，接着采用svm算法对各个候选框中的物体进行分类识别。也就是总个过程分为四个程序：
